import 'package:em_repair/pages/partten_lock_page.dart';
...
class _LockCodeState extends State<LockCode> {
  ...
  Future<void> _loadLockCode() async {
    ...
    setState(() {
      _storedLockCode = prefs.getStringList('lock_code')?.map(int.parse).toList() ?? [];
    });
  }
  ...
  Future<void> _saveLockCode(List<int> code) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setStringList('lock_code', code.map((e) => e.toString()).toList());
    setState(() {
            ...
            ElevatedButton(
              onPressed: () {
                ...
                if (text.isNotEmpty && text.length > 1) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('Please enter a valid lock code')),
                  );
                }
              },
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          // Display the stored lock code as an array of numbers
          ...
          const SizedBox(height: 16),
          // Display the stored pattern code
